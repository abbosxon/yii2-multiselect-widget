...
namespace abbosxon\widgets\multiselect;
...
/**
 * Class MenuMaker
 * @package abbosxon\widgets\multiselect
 *
...
class MultiSelect extends yii\bootstrap\Widget
{
    ...
    public $assetsClass = null;
    public $isJsRegistered = false;
    ...
    {
        if ($this->data === null || $this->selectedData === null || $this->addDataUrl === null || $this->removeDataUrl === null || $this->keyAttribute === null || $this->valueAttribute === null) {
            throw new InvalidConfigException(Yii::t('app', "Необходимо указать 'data','addDataUrl','removeDataUrl', 'keyAttribute', 'valueAttribute' и 'selectedData'"));
        } else {
    ...
    {
        $view = $this->getView();
        if ($this->assetsClass != null && class_exists($this->assetsClass)) {
            $class = $this->assetsClass;
            $class::register($view);
        } else
            MultiSelectAsset::register($view);

        if (!$this->isJsRegistered) {
            $js = <<<SCRIPT
